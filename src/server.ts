...
export interface ServerOptions {
	...
	shouldBindOnClose: (
		...
		socket: Socket,
		method: string,
		payload: Partial<RawProxiedRequest>
	) => Promise<boolean>;
	port: number;
	use_ssl: boolean;
	hostname?: string;
...
export class Server<IdentifyType extends ProxyIdentify = ProxyIdentify> {
	...
	emit<T extends keyof ServerEvents>(
		event: T,
		...data: Parameters<ServerEvents[T]>
	) {
		...
		return new Promise<void>(async (resolve, reject) => {
			...
			const onSocketResponse = (response: ProxiedResponse) => {
				...
				if (this.opts.debug)
					console.log(
						responseId,
						`<< Received after ${Date.now() - start}ms. Data :`,
						response
					);
			...
			const onRequestClosed = () => {
				if (!requestCompleted) {
					if (this.opts.debug)
						console.log(responseId, `<< Closed after ${Date.now() - start}ms`);
					socket.off(
						`${responseId}|${SOCKET_PROXIED_RESPONSE}`,
						onSocketResponse
					);
			...
			if (await this.opts.shouldBindOnClose(req, socket, method, payload))
	...
	async registerRoutes(routes: string[], socketId: string) {
		...
		const callbacks = routes.map((route) => {
			const [combined, method, path] = Array.from(
				PROXY_PATH_REGEX.exec(route)!
			);
			const callback = async (req: ExpressRequest, res: ExpressResponse) => {
				...
				const socket = this.socket.sockets.sockets.get(
					this.callbacks_to_sockets.get(funcHandle)!
				);
				if (socket && socket.connected) {
					...
					if (this.opts.debug)
						console.log('Failed to reach socket', socketId, 'on route', route);
					res.sendStatus(404);
				}
			};

			const finalRoute = route.trim().startsWith('-')
				? new RegExp(path)
				: `/${path}`;
	...
	start(callback?: () => void) {
		if (this.opts.use_ssl) {
			...
			this.server = https.createServer(
				{ key: this.opts.ssl_key, cert: this.opts.ssl_cert },
				this.app
			);
		} else {
